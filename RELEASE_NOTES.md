...
- Adds local scenario playbooks for:
  - OBS static website hosting and Flexus-style low-cost hosting.
  - ECS monitoring troubleshooting, EIP cost optimization, and IAM permission diagnostics.
  - CCI workload readiness, SWR image readiness, and FunctionGraph readiness.
  - Production Web/API readiness across compute, ELB, RDS, VPC/security groups, DNS/CDN/SCM/WAF, observability, backup, and cost.
  - MaaS usage governance and CCE cloud-native assessment.
- Adds MaaS usage and CCE assessment planners that generate non-executing evidence/request plans without reading or printing credentials.
- Adds `hcloud_acceptance_probe_run.py`, a built-in runner for HTTP, TCP, DNS, and TLS probe templates generated by the existing acceptance probe planner.
- Adds `hcloud_live_regression_plan.py` and `references/live-regression-runbook.md` so true-account validation can be planned with explicit inputs, evidence, and risk boundaries.
- Adds `hcloud_terraform_operations_plan.py` and `references/terraform/operations.md` for Terraform import, drift review, and remote-state planning with hcloud readback and explicit state-change gates.
- Updates scenario routing, script audience boundaries, Terraform references, and focused tests for the new local assets.

### Validation

- `python3 -m unittest discover tests`: 285 tests passed.
- `python3 -m compileall -q scripts tests`: passed.
- JSON parsing passed for `references/scenario-router.json`, `references/script-audience-manifest.json`, and `references/terraform/catalog/terraform-reference-catalog.json`.
- `git diff --check`: passed.

### Compatibility and Safety Notes

- v0.6.2 does not add Terraform apply automation, generic SDK mutation execution, or ungated cloud-resource mutation paths.
- The live probe runner executes only built-in HTTP/TCP/DNS/TLS checks from generated probe templates; it does not run arbitrary shell commands.
- Terraform import remains state-changing and is only executable behind explicit `--execute-import`, `--allow-state-change`, and confirmation-token gates.
- True-account regression remains user/environment dependent; local tests verify planners and contracts, not real Huawei Cloud account behavior.
