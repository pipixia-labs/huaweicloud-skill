...
- **Terraform 受控 IaC 面**：吸收 60 个 Terraform 示例和核心 provider/reference/inventory 文档，通过 `hcloud_terraform_context_inspect.py`、`hcloud_terraform_router.py` 和 `references/terraform/catalog/` 渐进选择资产。Terraform 不扩大 SDK allowlist，也不跳过 hcloud 现网发现、plan 确认和后置验证。
