...
3. v0.3 已从 ECS 单点闭环和多服务覆盖继续扩展到账号盘点、闲置审计、可观测、Billing/Cost request spec 和治理候选画像；v0.3.2 进一步把 VPC/安全组、EIP、EVS、ELB、RDS、OBS、DNS、SCM、CDN、CES/LTS 组织成 P0 任务级 lifecycle closure planner。准确 registry 服务数和 operation 计数以 `python3 scripts/hcloud_catalog_audit.py --pretty` 的 `registry` 字段为准。
