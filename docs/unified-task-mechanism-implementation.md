...
本文说明 `huaweicloud-skill` v0.9.0 已经实现的轻量大一统机制、运行边界和验证方法。它描述的是当前代码和资料，不是 plus 版的未来设计。
...
因此 v0.9.0 在 `SKILL.md` 中进一步强调：Agent 必须使用自身文件读写工具实际落盘，运行时待办和平台自动日志不能替代任务记录。这个修改提高了指令清晰度，但仍需后续真实运行继续验证遵循率。
