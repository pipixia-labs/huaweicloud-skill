...
本机制假定使用它的通用 Agent 能够读取 Skill、进行多轮对话、编写代码，并具备 workspace 文件读写和必要工具调用能力。Skill 只规定复杂任务需要记住什么，不规定具体怎么完成任务；不为缺少这些能力的 Agent 建设适配层。
...
因此，这套机制能够辅助具备上述前提的 Agent，但不能保证任何模型或运行时都一定遵守。当前架构不针对没有可写 workspace 的 Agent 提供持久任务记忆降级实现。
...
因此 v0.9.1 在 `SKILL.md` 中进一步强调：Agent 必须使用自身文件读写工具实际落盘，运行时待办和平台自动日志不能替代任务记录。
