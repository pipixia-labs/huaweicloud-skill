...
后续一次建站运行又暴露出副作用恢复缺口：用户目标只有一台站点 ECS，但 Agent 在删除 job 尚未核验终态时连续创建替代实例，最终出现三台同角色 ECS。当前 `main` 因此增加逻辑角色、预期数量、canonical 资源和待决操作摘要；明确异步结果先回读收敛、一换一替换、重复替换重新确认，以及私钥不进入普通 stdout/stderr 的边界。该修复仍需通过常见 Agent 的重复场景测试验证实际遵循率。
