...
2. 用 `hcloud_p2_scenario_closure_plan.py --group CCE` 获取场景级只读证据计划。
3. 用 `hcloud_resource_discovery.py --service CCE` 查询集群和基础资源线索。
4. 已知 cluster ID 后，用 `hcloud_resource_query.py` 查询目标集群详情。
5. 如涉及公网入口，继续按 ELB/VPC 指南验证 listener、member、security group 和协议探测。
