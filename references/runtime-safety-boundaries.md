...
- 用户要求部署、搭建、创建、开通、上线、绑定或修改资源时，除非用户只要方案咨询，否则不要停在泛泛步骤清单；先形成可核对方案并完成必要的只读现状检查、dry-run 计划或参数校验。
- 涉及计费资源、MaaS 图片/视频调用、上传、网络暴露或其他外部副作用时，必须先向用户展示方案、费用和风险边界，并等待用户对当前计划作出明确确认；由 agent 自行判断“风险已确认”不构成授权。确认必须与计划、范围和输入指纹绑定。
- **当前 M2.5 运行时规则**：通用 hcloud mutation、旧 guarded submit/dry-run、Terraform 状态变更和 MaaS 生成均强制返回 `plan_only`。在 Skill 内部的服务专用入口完成计划复验、明确确认、提交前事实刷新、调用和提交后回读前，禁止执行实际的 `Create*`、`Update*`、`Bind*`、`Attach*` 或 MaaS API 调用。`submission-authorization/v1` 当前只是本地准备记录；`controlled-submit-handoff/v1` 是已废弃的宿主交接试验，固定为 `plan_only`，不得被本地 `approval_id`、JSON 文件或旧 token 解释为真实提交授权。Skill 不要求任何 Agent 改代码来满足这些条件。
...
- 创建或修改安全组规则前，必须明确 `direction`、`protocol`、精确端口和来源 CIDR。公网 Web 方案仍要展示安全影响并取得用户确认；当前 guarded flow 只生成计划，不能借确认标记或旧 plan token 提交。
