...
- 创建前按逻辑角色和资源名做幂等查询。逻辑角色表示资源在当前目标中的用途，例如“个人站点主机”，名称变化不产生新的角色；发现同名或同角色候选时先读 `references/playbooks/resource-idempotency-reconcile.md`，选择 canonical resource 修复，不要继续创建。
- 对创建、删除、替换、绑定等付费或有副作用操作，在 task 记忆中按需保留同一逻辑角色的预期数量、canonical 资源、当前状态、待决操作和最近回读时间；这是轻量恢复摘要，不是云侧实时状态库。
- 同一逻辑角色存在待决或结果未知的 create/delete/replace/bind 时，先通过 job、资源列表、资源详情或等效证据回读收敛。没有确认前，不再创建、绑定或替换同角色资源，也不因命令退出码为 `0` 推断云侧已完成。
- 一换一替换必须形成收敛边界：除非用户已明确批准临时并存的迁移或高可用方案，否则先确认删除 job 成功且旧资源已不存在，再创建替代资源；创建后更新 canonical 资源并继续验证预期数量。
- 原方案已经向用户披露一次有界的一换一替换，且替换不增加预期数量、持续费用、安全暴露或破坏范围时，可以在该边界内继续。需要第二次替换、临时并存、扩大数量/费用/暴露面或改变破坏范围时，必须说明前次结果和新变化，重新说明并取得确认。
- 上述规则只规定安全不变量，不固定具体 API、参数或完整调用顺序。Agent 可以根据现场证据选择等待、查询、原地修复、替代路径或停止并说明阻塞。
...
- 新建、演示、测试、部署类资源默认可把重建/重装作为候选恢复路径，除非用户明确说要保留系统盘状态；“可重建”不是连续重建授权，也不表示可以在旧资源或异步结果尚未收敛时保留多份同角色资源。对于这类资源，不能因为缺 COC 就把任务停在云侧配置。
- ECS 登录或应用验收失败时，重建前至少核验创建 job 与实例状态、EIP/网络/安全组、所选凭据和登录方式、cloud-init、服务进程与日志中能够取得的证据。具体诊断深度和顺序由 Agent 按现场决定；只有证据表明原地修复不合理时，才进入受控替换。
- 对新建 ECS，优先用任务专用 keypair 保存 private key 并在创建后立即 SSH 验证。
- 创建 keypair 或其他 API 返回私钥时，必须让秘密直接进入权限受限的 artifact、凭据 broker 或 runtime secret 通道，不得经过普通 stdout/stderr、平台普通日志、task 文件或最终回复；当前 runtime 无法安全接收时停止并说明缺口。
