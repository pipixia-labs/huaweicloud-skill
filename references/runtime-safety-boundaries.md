...
- 用户要求部署、搭建、创建、开通、上线、绑定或修改资源时，除非用户只要方案咨询，否则不要停在泛泛步骤清单；先形成可执行方案并完成必要的只读现状检查、`--dryrun` 或参数校验。
- 涉及计费资源、MaaS 图片/视频调用、上传、网络暴露或其他外部副作用时，必须先向用户展示方案、费用和风险边界，并等待用户明确确认；由 agent 自行判断“风险已确认”不构成授权。收到确认后，才执行实际的 `Create*`、`Update*`、`Bind*`、`Attach*` 或 MaaS API 调用。
