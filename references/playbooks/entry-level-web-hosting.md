...
2. 需要简单后端或现成镜像时考虑 Flexus L/轻量服务器。
   ...
   - 当前 skill 没有独立 Flexus L 执行脚本；先把它作为产品选型建议，再用 ECS/VPC/OBS/DNS 现有 evidence path 做等价验证。
...
Flexus L 适合作为低门槛产品选型建议，但当前本 skill 不把它伪装成成熟执行面：

- 用 `entry-level-web-hosting` route 告诉用户它是“小预算简单后端”的候选。
- 如果需要实际创建、联网、登录和部署，按 ECS/VPC/SSH/COC 的已验证路径拆解能力需求。
- 不要承诺当前 skill 已有 Flexus L 专用创建脚本或完整参数映射。
- 如果用户坚持 Flexus L，先通过 hcloud metadata/help 或官方文档确认 service/operation，再生成 planner-only 方案。
